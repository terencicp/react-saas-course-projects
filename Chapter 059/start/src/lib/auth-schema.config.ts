...
// `emailAndPassword` block — which yields the four-table schema
// (user/session/account/verification); no plugin here adds tables.
//
// TODO(L2) — add organization() (schema-shaping only: teams off + invitation
// additionalFields) so auth:generate emits the org tables.
