...
// ── Plan store (chapter 104 audit target) ───────────────────────────────────
// The org records (carrying the customer-facing `planLabel`) and the per-org plan
// entitlements that the `/plan` overview surface reads. Seeded deterministically
// so `/plan` paints the same row on every boot with no login.
...
export const findPlanEntitlement = (
  orgId: string,
): PlanEntitlement | undefined =>
  planEntitlements.find((row) => row.orgId === orgId);
...
// The orgs that own invoices also own a plan. Seeded with a customer-facing
// `planLabel` the plan-label mutation rewrites.
const seedOrganizations = (): Organization[] => [
  { id: 'org-acme', name: 'Acme Northwind', planLabel: 'Team (annual)' },
  { id: 'org-globex', name: 'Globex Holdings', planLabel: 'Pro (monthly)' },
];

// The per-org plan entitlements the `/plan` overview reads. `renewsAt` is a fixed
// future instant (anchored well after ANCHOR) so the renewal countdown paints a
// stable, positive day count on every boot.
const seedPlanEntitlements = (): PlanEntitlement[] => [
  {
    orgId: 'org-acme',
    plan: 'Team',
    seatsAllocated: 25,
    seatsUsed: 18,
    renewsAt: new Date(ANCHOR + 90 * DAY).toISOString(),
  },
  {
    orgId: 'org-globex',
    plan: 'Pro',
    seatsAllocated: 10,
    seatsUsed: 4,
    renewsAt: new Date(ANCHOR + 45 * DAY).toISOString(),
  },
];
...
export const reseed = (): void => {
  ...
  organizations.push(...seedOrganizations());
  planEntitlements.length = 0;
  planEntitlements.push(...seedPlanEntitlements());
