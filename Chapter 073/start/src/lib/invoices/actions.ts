...
export const updateInvoice = authedAction(
  ...
  async (input, ctx): Promise<Result<Invoice>> => {
    ...
    // TODO(L3) — after commit, before redirect: updateTag list+record+summary, logCacheInvalidation(tag,'action')
    // TODO(L3) — misuse branch: when misuseRevalidateFromAction, revalidateTag the list tag instead of updateTag
