...
- `pnpm r2:cors` / `pnpm r2:lifecycle` — push the R2 bucket CORS rule / 7-day exports
  lifecycle rule (run by hand against your own bucket; never in the render pipeline).
...
assigns base62 ids, not UUIDs). Chapter 069 layers R2 on top: one `server-only`
`lib/r2.ts` client powers both the browser-PUT user uploads (`presignedPut` signs a
direct-to-R2 URL — the function is never a byte pipe) and the server-PUT export retrofit
(the worker PUTs the CSV directly). Two-step write (sign → PUT → `finalizeUpload` HEADs →
insert, never row-before-bytes); `byteSize`/`contentType` come from the HEAD, never the
client; object keys are server-constructed; `/files` mints fresh presigned GETs per render
(never cached). R2 is NOT in the render pipeline — the four `R2_*` env vars ship dummy
values so `getSignedUrl` (local HMAC) signs real-shaped URLs. No cache tags, no
dispatcher, no schedules/waitpoints/rate-limit.
