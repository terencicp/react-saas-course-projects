...
import { db } from '@/db';
import { exports, fileMetadata, invoices } from '@/db/schema';
...
export const tenantDb = (orgId: string) => ({
  query: {
    ...
    // The user-upload metadata rows. Every file read (getFile, getFileDownloadUrl,
    // listFiles) goes through here so the org predicate is the OUTER `and` — a
    // cross-org fileId resolves to null, and the caller's isNull(softDeletedAt)
    // filter rides as the inner `where`. The lone tenant-free path is the export
    // worker's getSignedGetForKey, which signs a raw key inside the trust boundary.
    fileMetadata: {
      findMany: ((config?: { where?: SQL }) =>
        db.query.fileMetadata.findMany({
          ...config,
          where: and(eq(fileMetadata.organizationId, orgId), config?.where),
        })) as typeof db.query.fileMetadata.findMany,
      findFirst: ((config?: { where?: SQL }) =>
        db.query.fileMetadata.findFirst({
          ...config,
          where: and(eq(fileMetadata.organizationId, orgId), config?.where),
        })) as typeof db.query.fileMetadata.findFirst,
    },
  },
