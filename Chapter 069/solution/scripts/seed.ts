...
// The seeded user-upload metadata rows — for org_acme ONLY (the other orgs get none,
// the tenancy-isolation proof: one org never sees another's files). Three live rows
// (png/jpg/pdf) render in the /files list with locally-signed presigned GET hrefs (the
// dummy R2 env lets getSignedUrl HMAC a real-shaped URL — no live bucket needed). One
// soft-deleted row (deleted-draft.webp) must NOT appear in the list (the soft-delete-
// read proof).
//
// Each id is a fixed, RFC-valid uuid (the same hex-prefix trick the invoice seed uses)
// so z.uuid() accepts it and the objectKey segment is valid; the row id IS the key
// segment. uploadedAt is descending so the newest-first list order is reproducible.
const FILE_ROWS = [
  {
    id: '00000000-0000-7000-8000-f11e00000001',
  ...
  {
    id: '00000000-0000-7000-8000-f11e00000002',
  ...
  {
    id: '00000000-0000-7000-8000-f11e00000003',
  ...
  {
    id: '00000000-0000-7000-8000-f11e00000004',
    originalFileName: 'deleted-draft.webp',
...
const FILE_ORG = 'org_acme';
const FILE_UPLOADER = 'user_alice';
const FILE_ANCHOR = Date.parse('2026-05-15T09:00:00.000Z');
...
export const runSeed = async (): Promise<void> => {
  ...
  // The user-upload metadata rows for org_acme only (drizzle-seed cannot seed the
  // unique-bearing file_metadata, so these are direct inserts with fixed RFC-valid
  // UUID ids). Three live + one soft-deleted; uploadedAt descends so the list order is
  // reproducible. The objectKey is built the same way buildObjectKey does (the row id
  // is the key segment, the extension comes from the content type).
  await dbUnpooled.insert(fileMetadata).values(
    FILE_ROWS.map((f, i) => ({
      id: f.id,
      organizationId: FILE_ORG,
      uploadedBy: FILE_UPLOADER,
      objectKey: `org/${FILE_ORG}/files/${f.id}.${f.ext}`,
      originalFileName: f.originalFileName,
      contentType: f.contentType,
      byteSize: f.byteSize,
      uploadedAt: new Date(FILE_ANCHOR - i * HOUR),
      softDeletedAt: f.softDeleted ? new Date(FILE_ANCHOR - i * HOUR) : null,
    })),
  );
