...
// TODO(L2) — uncomment + complete notifications / user_notification_preferences / notification_dedup per the shapes above; pnpm db:generate --name add_notifications
//
// The three notification tables. Until you uncomment + complete them, they do not exist:
// the inbox + inspector reads are guarded to render empty, and the dispatcher/channels/prefs/
// dedup stubs do not yet touch them. Fill the columns exactly as specified, then run
// `pnpm db:generate --name add_notifications` → review the SQL → `pnpm db:migrate`.
//
