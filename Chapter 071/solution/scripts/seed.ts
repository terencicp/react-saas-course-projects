...
export const runSeed = async (): Promise<void> => {
  ...
  // Notification preference fixtures (the inspector's deterministic scenarios). Bob
  // gets one team→email:false row (the suppression target); Alice gets NO row (the
  // default-on target via `?? true`); the other users stay without rows. Written via
  // raw SQL and guarded so the seed runs cleanly at scaffold (the table lands with the
  // student's S1 migration) — exercised once S1 creates user_notification_preferences.
  // No notifications / notification_dedup rows seeded: the inbox + dedup surfaces start
  // empty, and firing from the inspector populates them.
  try {
    await dbUnpooled.execute(sql`
      insert into user_notification_preferences (id, user_id, category, email, inbox, push)
      values (uuidv7(), 'user_bob', 'team', false, true, true)
      on conflict (user_id, category) do update
        set email = excluded.email, inbox = excluded.inbox, push = excluded.push
    `);
  } catch {
    // Table not present yet (pre-S1) — the prefs fixture lands once it exists.
  }
};
