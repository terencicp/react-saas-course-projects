...
//
// Past-due path: on the past_due transition (patch.status === 'past_due' on a non-stale
// UPDATE) this reads the org's owner user ids inside `tx` and PUSHES an
// org.billing.past_due descriptor onto the closure-captured pendingDispatches array
// route.ts owns. Collect-only — it never dispatches itself (read-inside, dispatch-after);
// route.ts runs the dispatch loop once db.transaction commits. The transactional-outbox
// alternative (a durable pending_dispatches table drained by a worker) is the next step.
