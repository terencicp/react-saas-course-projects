# Chapter 091 — Testing the Stripe webhook and Checkout money path

This is the starting code repo for the chapter 091 project of the React SaaS course.

This repo builds on the previous projects: 028, 035, 041, 047, 050, 055, 059, 062, 065.

The Chapter 065 Stripe-webhook app carried in **unchanged** as the system under test.
The deliverable is **test files, not app features**: three Vitest integration tests
against the real `POST /api/webhooks/stripe` handler (real test Postgres + per-test
rollback) and one Playwright spec driving the Upgrade-to-Pro money path against a
production build. The whole 065 app, both DB lifecycles, the MSW + auth + event + rollback
fixtures, and both test configs ship provided.

The four student files live as `TODO` stubs the lessons fill (`rg "TODO(L" tests`
enumerates the work). Everything else is the carried-in 065 app below.

---

## The system under test (carried from Chapter 065)

The `POST /api/webhooks/stripe` route that ingests three Stripe subscription events, and
the derived `plan_entitlements` row the app reads on every request.
...
## Testing (this chapter's deliverable)

Two independent test systems:

- **`pnpm test:lesson <n>`** — the lesson gate (`lesson-verification/Lesson <n>.ts`), a
  source-shape check the reviewer runs. Node env, no DB. Green pre-lesson (placeholders).
- **`pnpm test:integration`** / **`pnpm test:e2e`** — the student's real suite.

### Integration tests (Vitest, real test Postgres)

1. `docker compose up -d` — also starts the test-only Postgres (`postgres-test`, host
   port `55432`, holding both `saas_int_test` and `saas_e2e`).
2. `pnpm db:test:setup` — creates `saas_int_test` and applies the migrations.
3. `pnpm test:integration` — runs `tests/integration/**/*.int.test.ts`. Each test wraps in
   `withRollback`, so the suite runs green twice with no reset (zero rows left behind).
   Resend is mocked at the network boundary (MSW); Stripe's `subscriptions.retrieve` is
   stubbed at the SDK seam, while `webhooks.*` stay real — so **no live Stripe is needed**.

### E2E test (Playwright, production build)

1. `pnpm exec playwright install chromium` — one-time browser fetch (not a pnpm build
   step).
2. `cp .env.test.local.example .env.test.local` and fill in `STRIPE_SECRET_KEY` (your real
   **test-mode** key — the Checkout test creates a real session) and `E2E_ADMIN_PASSWORD`.
3. `pnpm db:e2e:reset` — creates/resets `saas_e2e`, migrates, and seeds the e2e org +
   `admin@e2e.test`.
4. In a separate terminal: `stripe listen --forward-to localhost:3001/api/webhooks/stripe`
   (the webhook delivery that flips the success page to Pro).
5. `pnpm test:e2e` — the `setup` project signs the admin in once (`.auth/admin.json`), then
   `chromium` drives the money path on the port-3001 production build.
