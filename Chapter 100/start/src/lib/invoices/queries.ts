...
// The row shape the list/detail return. The baseline carries the single combined
// `total` column (the anti-pattern the cadence fixes).
...
const orderBy = (sort: InvoiceSort) => {
  switch (sort) {
    ...
    case 'total':
      return [asc(invoices.total), asc(invoices.id)];
    case '-total':
      return [desc(invoices.total), desc(invoices.id)];
    case 'customer':
