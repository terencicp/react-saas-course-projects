...
// deleted row per org so the lifecycle views are non-empty. The baseline schema
// has the single combined `total` column (the anti-pattern the cadence fixes),
// so the seed sets `total` only; the expand-migrate-contract cadence introduces
// the subtotal/tax pair.
