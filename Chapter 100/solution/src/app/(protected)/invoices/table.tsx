...
export const InvoicesTable = ({
  ...
  // Optimistic archive: a row leaves the table the instant the user clicks. The
  // optimistic value expires when the action's transition ends; on `ok` the
  // revalidated `rows` no longer carries it, on refusal `rows` is unchanged and
  // the row reappears.
  ...
  // The menu's `onSelect` is a plain event handler, so the optimistic write must
  // share an explicit transition with the dispatch.
            ...
            <tr key={row.id} data-testid="invoice-row" className="border-b">
              ...
              <td className="py-2">
                ...
                {view === 'archived' && row.archivedAt ? (
                  ...
                  >
                    Archived on {row.archivedAt.toLocaleDateString()}
                  </div>
                ) : null}
              </td>
              <td className="py-2 capitalize">{row.status}</td>
              <td className="py-2 text-right tabular-nums">
                {row.currency} {combinedAmount(row)}
              </td>
