...
//
// Correlation (finding 3, slice S3): the proxy scope does NOT propagate into route
// handlers, so this handler recovers the `x-request-id` (or mints its own) and opens
// its own `runWithContext` scope — every log line then carries the requestId mixin and
// joins its Sentry event.
...
const handle = async (request: Request): Promise<Response> => {
  ...
  // No raw-header serialization (finding 2, slice S3): log only the intentional fields,
  // and let the single `redact` seam in lib/logger.ts catch anything a future field
  // accidentally carries. The `stripe-signature` signing material never reaches the logs
  // — the 3am rule holds.
