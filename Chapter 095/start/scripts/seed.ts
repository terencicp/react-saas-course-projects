...
// Seed emails use the @example.com synthetic domain (081 L4 rule) — never a real or
// `.test` address. Alice is the seeded admin/owner of org_acme the audit reads as.
const USERS = [
  { id: 'user_alice', name: 'Alice', email: 'alice@example.com' },
  { id: 'user_bob', name: 'Bob', email: 'bob@example.com' },
  { id: 'user_carol', name: 'Carol', email: 'carol@example.com' },
  { id: 'user_dave', name: 'Dave', email: 'dave@example.com' },
  { id: 'user_erin', name: 'Erin', email: 'erin@example.com' },
  { id: 'user_frank', name: 'Frank', email: 'frank@example.com' },
] as const;
...
const INVOICE_ORGS = [
  ...
  // non-hex id like "acme…"). `cuBase` is the matching tag for the org's customer
  // rows. `prefix` is the human invoice-number tag.
  {
    orgId: 'org_acme',
    prefix: 'ACME',
    idBase: 'ace0',
...
type CustomerSeed = typeof customers.$inferInsert;

// One real customer row per name per invoice-bearing org (Chapter 095). Each invoice
// references one via customerId — finding 8's N+1 loops over these, and the
// documented relations-API fix expands the relation. Customer ids are fixed, RFC-valid
// uuids per (org, index) so the seed is deterministic. `cuBase` is the org's 4-hex tag.
const buildCustomers = (orgId: string, cuBase: string): CustomerSeed[] =>
  INVOICE_CUSTOMERS.map((name, idx) => ({
    id: `00000000-0000-7000-9000-${cuBase}${String(idx + 1).padStart(8, '0')}`,
...
const buildInvoices = (
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const customerIdx = i % INVOICE_CUSTOMERS.length;
    rows.push({
      id: `00000000-0000-7000-8000-${idBase}${String(i).padStart(8, '0')}`,
      organizationId: orgId,
      customerId: `00000000-0000-7000-9000-${cuBase}${String(customerIdx + 1).padStart(8, '0')}`,
      number: `${prefix}-${String(1000 + i)}`,
      customerName: INVOICE_CUSTOMERS[customerIdx] ?? 'Acme',
...
// The seeded invoice the audit reads against, and the planted note whose body
// carries tame markup. notes.tsx renders this via dangerouslySetInnerHTML with no
// sanitization (SEEDED #2), so the running app paints `<b>bold</b>` as LIVE bold —
// the visible XSS-sink fingerprint at /invoices/<this id>.
const SEED_INVOICE_ID = '00000000-0000-7000-8000-ace000000001';
const SEED_NOTE = {
  id: '00000000-0000-7000-8000-aaaa00000001',
  invoiceId: SEED_INVOICE_ID,
  organizationId: 'org_acme',
  authorId: 'user_bob',
  body: 'Customer asked us to mark this <b>bold</b> — follow up next week.',
} as const;
...
export const runSeed = async (): Promise<void> => {
  ...
  // Point each org at its owner member (the ownerId column the ownership-transfer
  // helpers mutate — findings 1/3). The owner is the first 'owner' member per org.
  ...
  // A free entitlement row per org so getEntitlement() always finds one (the
  // provisioning invariant: every org gets a free row at creation/seed).
  ...
  // Customers first (invoices FK → customers.id), then 200+ invoices per
  // invoice-bearing org (org_empty intentionally gets none). Inserted in chunks so
  // the parameter count stays sane.
  for (const { orgId, prefix, idBase, cuBase, count } of INVOICE_ORGS) {
    await dbUnpooled.insert(customers).values(buildCustomers(orgId, cuBase));

    const rows = buildInvoices(orgId, prefix, idBase, cuBase, count);
    for (let i = 0; i < rows.length; i += 100) {
      await dbUnpooled.insert(invoices).values(rows.slice(i, i + 100));
    }
  }

  // The planted invoice note carrying tame markup (SEEDED #2). notes.tsx renders the
  // body via dangerouslySetInnerHTML, so /invoices/<SEED_INVOICE_ID> paints it as
  // live bold HTML — the visible XSS-sink fingerprint.
  await dbUnpooled.insert(invoiceNotes).values({
    id: SEED_NOTE.id,
    invoiceId: SEED_NOTE.invoiceId,
    organizationId: SEED_NOTE.organizationId,
    authorId: SEED_NOTE.authorId,
    body: SEED_NOTE.body,
    createdAt: NOW,
  });
  ...
  console.info(
    '[seed] seeded invoice with planted XSS note:',
    `/invoices/${SEED_INVOICE_ID}`,
  );
};
