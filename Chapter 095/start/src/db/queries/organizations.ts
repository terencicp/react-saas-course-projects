...
// The active-org row read the dashboard awaits (Chapter 095). One row by id; throws
// if missing (a provisioning invariant — the active org always exists). The
// dashboard's RSC waterfall awaits this AFTER the user read (a genuine dependency:
// the orgId comes from the session), so user → org stays sequential.
