...
// A serializable projection of `Invoice` for the client table: `Temporal.Instant`
// / `Temporal.PlainDate` are class instances and cannot cross the RSC → Client
// boundary, so they ride as primitives (`createdAtMs` epoch millis, `dueDateISO`
// the calendar-day string). The formatter cells (S2) reconstruct as needed —
// `new Date(row.createdAtMs)` with the profile `timeZone`, `row.dueDateISO` for
// the calendar day, `row.amountMinor` / 100 for the amount.
export type InvoiceRow = Omit<Invoice, 'createdAt' | 'dueDate'> & {
  createdAtMs: number;
  dueDateISO: string;
};

export const toInvoiceRow = (invoice: Invoice): InvoiceRow => {
  const { createdAt, dueDate, ...rest } = invoice;
  return {
    ...rest,
    createdAtMs: createdAt.epochMilliseconds,
    dueDateISO: dueDate.toString(),
  };
};

// `createdAt` is a `Temporal.Instant`; compare via `Temporal.Instant.compare`
// (returns -1/0/1) rather than string `localeCompare`.
