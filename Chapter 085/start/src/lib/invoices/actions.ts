...
import { type InvoiceRow, toInvoiceRow } from '@/lib/invoices/queries';
...
// Actions return the serializable `InvoiceRow` (not the domain `Invoice`): the
// Result rides back to the client through `useActionState`, and `Temporal.Instant`
// / `Temporal.PlainDate` instances cannot cross that boundary.
...
export const updateInvoice = authedAction(
  ...
  async (input, ctx): Promise<Result<InvoiceRow>> => {
    ...
    if (!input.overwrite && row.version !== input.version) {
      return conflict(CONFLICT_MESSAGE, toInvoiceRow(row));
    }
