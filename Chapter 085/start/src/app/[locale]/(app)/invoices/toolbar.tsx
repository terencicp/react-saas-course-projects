...
export const Toolbar = ({ parsed }: { parsed: ListParsed }) => {
  // TODO(L2) — route every visible string here through
  // `useTranslations('invoices.list')`: the status placeholder + "All statuses"
  // (`toolbar.statusPlaceholder`/`toolbar.statusAll`), the status option labels
  // (reuse the existing `status.*` keys), the sort placeholder + option labels
  // (`toolbar.sortPlaceholder`/`toolbar.sort.*`), and the "Search…" placeholder
  // (`toolbar.searchPlaceholder`); no hard-coded JSX strings under [locale]/.
