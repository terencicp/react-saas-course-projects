...
// The direct-input twin of `authedAction`. Same pipeline (session → authorize →
// parse → `fn`, throws become an `internal` Result), but it takes a plain object
// instead of FormData and returns `(input) => Promise<Result>` — the shape a
// TanStack `useMutation` can call as `mutationFn`. The FormData wrapper above is
// for `useActionState`; this one is for `useMutation`.
