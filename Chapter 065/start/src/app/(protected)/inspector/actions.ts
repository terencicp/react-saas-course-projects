...
export const forceEntitlementStatus = async (
  ...
  // At scaffold the plan_entitlements table is PK-only, so the forced columns have
  // nowhere to land yet — the UPSERT writes the PK alone (the panel renders the
  // getEntitlement placeholder). Once S3 adds the columns, drop the `void forced` and
  // write `.values({ organizationId: orgId, ...forced }).onConflictDoUpdate({ set:
  // forced })` so the debug projects a realistic `pro`/`team` row.
  void forced;
  await db
